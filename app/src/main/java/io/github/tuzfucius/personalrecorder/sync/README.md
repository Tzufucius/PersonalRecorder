...
Reconcile 遵循 Pull Before Push：远端-only 文件下载并校验后才落盘；manifest 有 SHA 时先校验正文，旧 manifest 记录 `LEGACY_UNVERIFIED`；本地-only 文件上传；同路径不同内容按 JSONL 集合合并；相同事件 ID 内容不一致时写入 `archive_conflicts` 和原始文件，不静默覆盖。409 只允许重新发现并协调一次。
